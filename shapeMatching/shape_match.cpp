...
namespace shapematch{
	...
	void ShapeMatching::MakingTemplates(Mat modelRGB, AngleRange angle_range, ScaleRange scale_range,
		...
		//转灰度
		cv::Mat model;
		cvtColor(modelRGB, model, COLOR_BGR2GRAY);
		//对容器进行一个清除操作
		ClearModel();
		...
		for (int p = 0; p <= PyramidLevel_7; p++) {
			...
			for (int s = 0; s < shape_infos.size(); s++) {
				...
				//	//画出轮廓点;
				//	line(modelRGB,
				//		Point(feature.x+25, feature.y+20),
				//		Point(feature.x + 25, feature.y + 20),
				//		cv::Scalar(255, 0,0), 3);
				//}
				//for (int r = 0; r < modelRGB.rows; r += 13)
				//{
				//	cv::line(modelRGB, Point(0, r), Point(modelRGB.cols, r), cv::Scalar(0, 0, 255), 1, 8, 0);
				//}
				//for (int c = 0; c < modelRGB.cols; c += 13)
				//{
				//	cv::line(modelRGB, Point(c, 0), Point(c, modelRGB.rows), cv::Scalar(0, 0, 255), 1, 8, 0);
				//}
	...
	vector<Match> ShapeMatching::Matching(Mat source, float score_thresh, float overlap,
	...
	//进行扩充，方便模板图像旋转放缩
	void ShapeMatching::PaddingModelAndMask(Mat &model, Mat &mask, float max_scale) {
...
#pragma omp parallel for
	...
	//双阈值设置提取边缘，canny算子
	Template ShapeMatching::ExtractTemplate(Mat angle, Mat quantized_angle, Mat mag, ShapeInfo shape_info,
		...
		if (candidates.size() >= num_features && num_features > 0) {
			...
			//网格化筛选
			std::vector<Candidate>newCandidates = gridSelectFeaturePoints(angle, quantized_angle, mag, candidates);
			std::vector<cv::Point2f>vPt;
			for (int c = 0; c < candidates.size(); c++) {
				vPt.push_back(cv::Point2f(candidates[c].feature.x, candidates[c].feature.y));
	...
	//网格化筛选特征点
	std::vector<Candidate> ShapeMatching::gridSelectFeaturePoints(cv::Mat angle,
		cv::Mat quantized_angle,cv::Mat mag, std:: vector<Candidate> candidates)
	{
	...
	//裁剪模板
	Rect ShapeMatching::CropTemplate(Template &templ) {
...
//	//量化8方向匹配
//	vector<Match> ShapeMatching::MatchingPyrd8(Mat src, PyramidLevel pl, vector<int> region_idxes) {
//
//		vector<Match> matches; matches.clear();
//		Mat angle, quantized_angle, mag;
